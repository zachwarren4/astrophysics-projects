...
def dist(loc1,loc2,x,y,z):
    return np.sqrt((x[loc1] - x[loc2])**2 + (y[loc1] - y[loc2])**2 + (z[loc1] - z[loc2])**2)

def countForBin(bin,dataPoints,x,y,z):
    finalCount = np.linspace(0,0,15)
    i = 0
    while i < dataPoints + 2:
        for j in range(i + 1,dataPoints):
            w = int(round((np.sqrt((x[i] - x[j])**2 + (y[i] - y[j])**2 + (z[i] - z[j])**2)+1)/1.436))
            if w < 15:
                finalCount[w]+=1
        i = i + 1
    return finalCount
...
def correlation(data, rand,dataPoints):
    ...
    x = dfD.apply(lambda row: toX(row), axis = 1)
    y = dfD.apply(lambda row: toY(row), axis = 1)
    z = dfD.apply(lambda row: toZ(row), axis = 1)

    kd1 = spatial.KDTree(list(zip(x,y,z)))
    print(list(zip(x,y,z)))
    print(kd1.count_neighbors(kd1,r=.1))

    xr = dfR.apply(lambda row: toX(row), axis = 1)
    yr = dfR.apply(lambda row: toY(row), axis = 1)
    zr = dfR.apply(lambda row: toZ(row), axis = 1)


    bins = np.linspace(.1,20,15)
    #print(bins)
    countD = countForBin(bins,dataPoints,x,y,z)
    countR = countForBin(bins,dataPoints,xr,yr,zr)
...
def correlationDM(data, rand,dataPoints):
    ...
    bins = np.linspace(.1,20,15)
    #print(bins)
    countD = countForBin(bins,dfD,dataPoints)
    countR = countForBin(bins,dfR,dataPoints)

    dfD.to_csv('dataDM.csv')
...
corr20,bins20 = correlation('SDSS_Mr20_rspace.dat','SDSS_random.dat',100)
corr21,bins21 = correlation('SDSS_Mr21_rspace.dat','SDSS_random.dat',100)
corr20z,bins20z = correlation('SDSS_Mr20_zspace.dat','SDSS_random.dat',100)
#corrDM,binsDM = correlationDM('DM.dat','DM_random.dat',100)
